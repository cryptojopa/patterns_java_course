...
1) Разные алгоритмы расчета метрик легко добавлять без изменения существующего кода
2) Логика расчета метрик выделена в отдельные стратегии, что снижает сложность и уменьшает связность кода.
3) Разные подходы к расчету метрик реализуются изолированно, тем самым предотвращая копирование кода 
4) Отдельные стратегии легко тестировать независимо от остальной системы